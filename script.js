(function () {
  ...
  function localizeDigitsInText(el) {
    if (!el) return;
    var t = el.textContent;
    if (!t) return;
    el.textContent = toArabicIndicDigits(t);
  }
  ...
  var bgPresets = ["default", "blush", "midnight", "sage"];

  function applyBgPreset(preset) {
    if (!preset) preset = "default";
    if (bgPresets.indexOf(preset) === -1) preset = "default";
  ...
  function initBg() {
    ...
    applyBgPreset(stored || "default");
  }

  function cycleBg() {
    var current = document.documentElement.getAttribute("data-bg") || "default";
    var idx = bgPresets.indexOf(current);
    var next = bgPresets[(idx + 1) % bgPresets.length];
    applyBgPreset(next);
  }

  initBg();
  if (bgToggle) {
    bgToggle.addEventListener("click", cycleBg);
  }
  ...
  function tickCountdown() {
    ...
    function setVal(el, val, key) {
      ...
      if (prevVals[key] !== str) {
        prevVals[key] = str;
        el.textContent = toArabicIndicDigits(str);
  ...
  // Convert visible date/time digits across the page (without touching form inputs)
  document.querySelectorAll(".timeline__date, .hero__date, .detail-card__text").forEach(function (el) {
    localizeDigitsInText(el);
  });
